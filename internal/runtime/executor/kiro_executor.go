...
// 3. kiroauth.DefaultKiroRegion (us-east-1) - fallback
...
func resolveKiroAPIRegion(auth *cliproxyauth.Auth) string {
	if auth == nil || auth.Metadata == nil {
		return kiroauth.DefaultKiroRegion
	}
	...
	log.Debugf("kiro: using region %s (source: default)", kiroauth.DefaultKiroRegion)
	return kiroauth.DefaultKiroRegion
}
